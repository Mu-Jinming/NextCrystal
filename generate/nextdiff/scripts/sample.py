...
# Resolve public CLI paths relative to the caller's working directory.
...
def resolve_cli_path(path):
    if os.path.isabs(path):
        return path
    return os.path.abspath(os.path.join(_INVOCATION_CWD, path))

def main(args):


    tar_dir = resolve_cli_path(args.save_path)
    os.makedirs(tar_dir, exist_ok=True)

    if args.json_file != '':
        dataset = construct_dataset_from_json(resolve_cli_path(args.json_file))
    ...
    structure_list = generate_structures_from_dataset(
        resolve_cli_path(args.model_path),
        dataset,
        args.batch_size,
        args.step_lr,
    )
